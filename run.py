"""
Module Description: This module contains functions for"
...
def validate_name_country(input_string):
    """
    Validates whether the input string contains only"
...
def validate_password(input_string, mode):
    ...
    Returns:
    - bool: True if the password input is valid"
    " according to the specified difficulty level, False otherwise.
    """
    if mode == 'C':
        return all(char.isdigit() and int(char) <= 3 
                   for char in input_string.strip().split())
    if mode == 'E':
        return all(char.isdigit() and int(char) <= 5 
                   for char in input_string.strip().split())
    if mode == 'H':
        return all(char.isdigit() and int(char) <= 9 
                   for char in input_string.strip().split())
    return False
...
print("- The numbers in the password are within a specific range depending on "
...
print("- If your guess contains the correct number at the correct position,"
      "it will be revealed.")
...
while True:
    user_mode = input("Choose the game mode - 'C' for child, 'E' for easy,"
                      " or 'H' for hard: ") \
             .strip() \
             .upper()
...
while True:
    ...
    # Get user's guess
    guess = input("Enter your guess (6 numbers separated by spaces,"
    ...
    if not validate_password(guess, user_mode):
        print(f"Please enter valid numbers within the difficulty range "
              f"{DIFFICULTY_RANGE}.")
        continue
    ...
    if CORRECT_NUMBERS == 6:
        ...
        if correct_positions:
            print(f"Correct number/s found at position/s: "
                  f"{', '.join(map(str, correct_positions))}")
        else:
            print("Incorrect guess. Try again.")
        GAME_OUTCOME = 'Lost'
...
if GAME_OUTCOME != 'Quit':
    player_info = [player_name, player_country,
                   user_mode, GAME_OUTCOME, elapsed_time]
...
for row in leaderboard_data:
    print(f"{row[0]:<10} {row[1]:<14} {row[2]:<8} {row[3]:<8} {row[4]}")
